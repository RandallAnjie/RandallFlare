...
| R2 objects | partial | native binding + get/head/put/delete/list, metadata, ranges, conditions and delimiter cursors are done; public-domain and S3 GET/HEAD/Range use bounded-memory verified local files or full-SHA-256 rclone read spools with abort/crash cleanup; Worker-binding, upload-body and cross-node multi-GiB streams remain |
| R2 multipart | partial | create/upload/complete/abort, ETags, cursor-paged active-upload inventory, part inspection, staged-byte accounting, operator abort/GC, disk-spooled constant-memory assembly, whole-object SHA-256 verification, file-streamed rclone publication and stale-spool cleanup are implemented across encrypted API, CLI and Chinese console; multi-GiB request/response streaming, local-replica streaming and provider compatibility soak remain |
...
| rclone backend | partial | credential-isolated config, bounded cluster-wide remote probes, verified delete, byte-buffer writes, file-streamed `rcat` publication, fail-closed full-digest read spooling and bounded public/S3 response streams are done; cross-node large-object transport and an optional persistent read cache remain |
