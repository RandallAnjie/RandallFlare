...
| Workflow | partial | signed definitions, per-Workflow D1 quorum, idempotent definition-pinned instances, exactly-once replay boundaries, pinned retry/timeout/entrypoint policy, durable sleep/sleepUntil, signals, fenced leases and heartbeats, crash replay, atomic concurrency caps, UTC Cron, hashed-token Webhooks with default/custom TLS hostnames, pause/resume/terminate/restart, retention, Worker binding, encrypted API, CLI, audit timeline and Chinese console are implemented; named concurrency groups and multi-node crash-point soak remain |
