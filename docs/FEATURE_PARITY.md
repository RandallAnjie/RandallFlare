...
| rclone backend | partial | credential-isolated config, bounded cluster-wide remote probes, verified read/delete, byte-buffer writes and file-streamed `rcat` publication are done; streamed reads, cross-node large-object transport and read cache remain |
