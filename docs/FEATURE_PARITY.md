...
| device/exit networking | partial | one-way signed enrolment, capability-tagged exits, local HTTP plus SOCKS5 TCP/UDP split routing, reusable authenticated TLS UDP tunnels, per-datagram server-side policy/DNS/SSRF checks and Chinese console are implemented; OS TUN adapters remain |
