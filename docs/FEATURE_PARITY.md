...
| R2 credentials/grants/S3 API | partial | sealed Secret Access Keys, signed revocation/rotation, full-account or per-bucket grants, path-style Signature V4, streamed checksum-verified PUT, conditional/ranged GET, multipart create/upload/list-parts/list-uploads/complete/abort are implemented; CopyObject, presigned URLs, SDK compatibility matrix and production soak remain |
