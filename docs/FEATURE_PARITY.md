...
| R2 objects | partial | native binding + get/head/put/delete/list, metadata, ranges, conditions and delimiter cursors are done; Worker binding, public-domain and S3 GET/HEAD/Range use bounded-memory verified files; unavailable local/rclone bytes are borrowed through sequence-bound XChaCha20-Poly1305 frames, authenticated EOF and final size/SHA-256 verification, with two-node repair and real-workerd multi-frame reads tested; streaming upload bodies remain |
| R2 multipart | partial | create/upload/complete/abort, ETags, cursor-paged active-upload inventory, part inspection, staged-byte accounting, operator abort/GC, disk-spooled constant-memory assembly, whole-object SHA-256 verification, file-streamed rclone publication, Worker/public/S3 response streaming and stale-spool cleanup are implemented across encrypted API, CLI and Chinese console; request-body streaming, multi-node local large-object writes and provider compatibility soak remain |
...
| rclone backend | partial | credential-isolated config, bounded cluster-wide remote probes, verified delete, byte-buffer writes, file-streamed `rcat` publication, fail-closed full-digest read spooling, bounded Worker/public/S3 responses and authenticated cross-node large-object reads are done; an optional persistent read cache and provider fault soak remain |
