...
| device/exit networking | partial | one-way signed enrolment, capability-tagged exits, local HTTP plus SOCKS5 TCP/UDP split routing, reusable authenticated TLS UDP tunnels, per-datagram server-side policy/DNS/SSRF checks and Chinese console are implemented; the embedded Linux whole-device TUN has virtual DNS, marked self-resolution, management/SSH/VPN bypass, isolated routing, dead-man cleanup and real-kernel TCP/UDP namespace E2E coverage; Apple Network Extension packaging and production multi-region soak remain |
...
| D1 replicated SQL | partial | Worker prepare/bind/all/first/run/raw/exec/batch/withSession, Raft-atomic mixed read/write batch, Chinese schema/data/metrics browser, bounded SQL import, leader-consistent portable SQLite export, independently authenticated encrypted export streams, constant-memory 10-GiB R2/rclone backups, signed schedules, deterministic live-node execution, file/multipart publication and retention cleanup are implemented and real-daemon tested; continuous point-in-time recovery and multi-VPS fault soak remain |
| Durable Objects | partial | the pinned 2026-08-04 native namespace/ID/stub/RPC, async and sync KV, SQL/cursors, transactions, alarms, state/concurrency and Hibernation surfaces are real-workerd tested under a per-Worker quorum, including background snapshots, restart/owner-loss recovery, encrypted cross-node WebSocket tunnels, automatic responses, tags/attachments and reconnect-after-failover; Cloudflare's platform-only jurisdiction enforcement and 30-day PITR log are explicitly not claimed, while long-lived multi-VPS socket soak and online owner migration remain |
...
| visual Flow | partial | signed/version-frozen DAGs, Vercel-style Chinese canvas and inspector, manual/webhook/cron triggers, one-way token hashes, synchronous Flow-as-API, idempotency, per-Flow D1 run/step/audit ledger, fenced recovery, cancellation/retry/retention/concurrency, durable loop subgraphs, typed full JSONata with bounded evaluator resources, guarded inline subflows, SSRF-safe HTTP/local credentials, failure alerts, and Worker/KV/D1/R2/Queue/Analytics/Pipeline/Workflow/Email nodes are implemented; nested loops inside subflows remain intentionally aligned with the reference's documented exclusion, while multi-node crash-point soak remains |
...
| client devices and exit rules | partial | one-way signed device credentials, Surge/Clash parsing, immutable RULE-SET/GEOIP snapshots, DIRECT/REJECT/specific/nearest routing, local SOCKS5 TCP/UDP plus HTTP proxy, reusable TLS UDP associations, gossip capability discovery, strict per-datagram server-side policy replay and SSRF blocking, CLI and Chinese console are implemented; Linux whole-device TUN now covers IPv4/IPv6, virtual DNS, marked direct/exit sockets, management safety and TCP/UDP kernel E2E; Apple Network Extension packaging and production multi-region latency soak remain |
