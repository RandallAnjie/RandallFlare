...
- 出站直接查询收件域 MX，按优先级尝试；无 MTA-STS 策略时使用机会式 STARTTLS，
  Null MX 会永久拒绝；
- 收件域发布 `_mta-sts.<域名>` 后，平台按 RFC 8461 从固定 HTTPS 地址读取策略。
  策略下载要求可信证书、`text/plain`、不跟随重定向，限制为 64 KiB，并拒绝解析
  到本机、内网、链路本地或特殊地址；失败抓取至少退避五分钟；
- MTA-STS 策略按 `max_age` 缓存在邮件域的 D1 法定人数账本中，节点切换或重启后
  仍可使用。`enforce` 只投递至单层通配符或精确模式匹配的 MX，并强制 STARTTLS
  与主机证书验证；`testing` 允许按普通投递继续，但会生成审计事件；
...
- 达到签名 `retention_days` 后，只有终态记录会被清理；仍被其他收件人引用的共享
  原文不会提前删除；
- 后台可以查看投递状态、远端 SMTP 响应、MTA-STS 模式/策略 ID/验证结果、入站
  认证结果和 R2 键，并下载原始 `.eml`。
