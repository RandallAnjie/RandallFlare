...
设备运行 `rf device proxy`，在回环地址同时提供 SOCKS5 与 HTTP 代理。它通过 HTTPS
...
跨网段流量采用 TLS 内的标准 SOCKS5 用户名/密码认证。用户名是设备 ID，密码是
...
浏览器或系统可以把 `127.0.0.1:7388` 同时配置为 SOCKS5 与 HTTP 代理。普通 HTTP
代理支持绝对 URL，HTTPS 使用 `CONNECT`。本地监听必须保持在回环地址；如需为整个
