...
multipart 的完成与发布已是有界内存的文件流；公网、S3、Worker binding 和节点借用
路径的多 GiB 请求/响应流仍在兼容性清单中分别跟踪。本地单节点后端最多组装
512 MiB；本地多节点的大对象在流式副本协议完成前会安全拒绝，生产大对象应使用
rclone 策略。存储策略不会替代后端自身的版本控制、跨区域复制和备份策略。
