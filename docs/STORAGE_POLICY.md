...
当前直接上传和单个 part 上限均为 5 GiB，每个上传最多 10,000 个 part，multipart
实际对象上限为 5 TiB 减 5 GiB。除末片外，每片至少 5 MiB 且大小一致；末片可更小。
分片 ETag 是内容 MD5，完成 ETag 是各分片二进制 MD5 拼接后的 MD5 加 `-分片数`。
未完成上传默认保留七天。rclone multipart 的完成与发布已是有界内存的文件流。公网 R2 域名、S3 GET/HEAD/Range
和原生 Worker R2 binding 读取都会先获得完整校验的本地文件，再以有界内存输出。
远端对象会流式落入临时 spool，核对完整 SHA-256 后才允许构造响应；客户端中断会
立即删除 spool，进程崩溃残留由生命周期任务回收。这优先保证“不返回未经验证的
字节”，首次读取会多一次落盘等待。

当入口或 Worker 节点没有具体的本地/rclone 字节时，会向具备能力的节点借用精确
对象版本。Peer 响应拆成 1 MiB 帧：每帧使用独立随机 nonce，把请求会话、HTTP
状态和严格递增序号绑定到 XChaCha20-Poly1305 关联数据，并以认证 EOF 帧结束。
接收端边解密边落盘，拒绝缺帧、重放/乱序、截断和尾随帧；只有最终大小与 SHA-256
都符合多数派元数据时才开放 spool。

S3、原生 Worker R2 binding 与节点借用请求都会把上传体按块写入临时文件，同时计算
SHA-256 与 MD5；摘要和大小通过后才提交对象元数据。节点间请求使用与读取对称的
逐帧 XChaCha20-Poly1305 协议，帧额外绑定时间戳、方法、完整路径、目标节点和会话，
因此本地多数派副本及无 rclone 能力节点的转交都不需要整包内存。单节点与多节点本地
存储采用同一对象上限。存储策略不会替代后端自身的版本控制、跨区域复制和备份策略。
