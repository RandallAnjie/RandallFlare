...
节点按各 part 的实际位置读取、校验并组装最终对象；旧版本数据库会在线增加位置列，
并用上传会话原有的位置回填。中止、过期清理和引用安全的延迟回收也使用被固定的
位置，不会套用后来变化的策略。
...
当前直接上传上限为 63 MiB，较大对象使用 multipart；真正的多 GiB 流式请求体仍在
兼容性清单中跟踪。存储策略不会替代后端自身的版本控制、跨区域复制和备份策略。
