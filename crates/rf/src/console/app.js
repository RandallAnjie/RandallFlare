...
async function openEmailMessage(id) {
  ...
  try {
    ...
    const fields = [["方向", message.direction === "inbound" ? "入站" : "出站"], ["状态", emailStatusLabel(message.status)], ["信封发件人", message.mail_from || "空"], ["信封收件人", message.rcpt_to], ["大小", formatBytes(message.size)], ["尝试次数", message.attempts], ["投递状态通知（DSN）", emailDsnStatusLabel(message.dsn_status)], ["DSN 记录", message.dsn_message_id || "无"], ["DSN 尝试 / 错误", `${message.dsn_attempts || 0} / ${message.dsn_last_error || "无"}`], ["SPF / DKIM / DMARC / ARC", [message.spf, message.dkim, message.dmarc, message.arc].filter(Boolean).join(" / ") || "无"], ["R2 对象", message.object_key], ["SHA-256", message.sha256], ["最后错误", message.last_error || "无"]];
